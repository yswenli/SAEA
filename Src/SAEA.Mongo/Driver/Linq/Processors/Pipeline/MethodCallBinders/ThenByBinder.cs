...
namespace SAEA.Mongo.Driver.Linq.Processors.Pipeline.MethodCallBinders
{
        ...
        public Expression Bind(PipelineExpression pipeline, PipelineBindingContext bindingContext, MethodCallExpression node, IEnumerable<Expression> arguments)
        {
            ...
            return new PipelineExpression(
                new OrderByExpression(
                    source.Source,
                    source.Clauses.Concat(new[] { new SAEA.Mongo.Driver.Linq.Expressions.OrderByClause(ordering, direction) })),
                pipeline.Projector);
        }
    }
}
